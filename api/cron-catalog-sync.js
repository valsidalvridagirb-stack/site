// Заміняє собою колишній ручний ланцюжок Google Drive catalog xlsx ->
// scripts/sync_catalog.py -> git push: тепер порівнює вже готовий
// supplier_catalog_merged (SQL-в'юха над supplier_catalog, яку наповнюють
// 5 cron-catalog-<постачальник>.js ендпоінтів) напряму з таблицею products
// у Supabase й одразу пише зміни через сервісний ключ — без git-кроку.
//
...
// api/_data/excel-catalog.json — /api/catalog.js (після Task #25) читає
// supplier_catalog_merged напряму, тож статичний файл більше не потрібен.
const { supaService, supaServiceAll } = require('./_lib/supabaseService');
